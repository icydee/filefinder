...
sub setup {
    my $self = shift;
$self->log->debug(qq/Before NEXT/);
    $self->NEXT::setup(@_);
$self->log->debug(qq/After NEXT/);
    my $calldir = $self;
    $calldir =~ s#::#/#g;
    my $file = "$calldir.pm";
    my $path = $INC{$file};
    $path =~ s#\.pm$#/I18N#;
    $self->log->debug("self = [$self] path = [$path] file = [$file]");
    eval <<"";
      package $self;
    ...
    if ($@) {
        ...
        $self->log->debug(qq/Initializedddd i18n "$self\::I18N"/) if $self->debug;
    }
}
